module Crawler::Scrapers
  class CamaraDeputados::RecibosDespesas < Base

    def capturar
      deputados_select_ids = coleta_nu_deputado_id
      Deputado.all.each do |deputado|
        abre_pagina_recibos(deputado, deputados_select_ids)
      end
    end

    private

    def coleta_nu_deputado_id
      deputados = Hash.new
    ...
    def abre_pagina_recibos(deputado, deputados_select_ids)
    ...
    def pega_links_dos_recibos(page)
      page.css('#content table tr').each do |item|
        if item.css('td:nth-child(3) > a').count >= 1
          ...
          salva_despesa(filtro, 'http://www.camara.gov.br' + item.css('td:nth-child(3) > a').attribute('href').text.strip)
        end
      end
    end

    def salva_despesa(filtro, link_despesa)
