	...
	def self.parse_cota_parlamentar(content)
    	...
    	despesas = doc.css("DESPESA")
			despesas.map do |despesa|
				if !despesa.css("ideCadastro").text.blank? and !despesa.css("cpf_cnpj").text.blank? and !despesa.css("numero").text.blank?

					deputado = Rails.cache.fetch("deputados", expires_in: 4.year) do
      			Deputado.find(despesa.css("ideCadastro").text)
    			end
